...
contract Claims is IBridgeStructs, Utils, Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function _submitClaimsBRC(BridgingRequestClaim calldata _claim, uint256 i, address _caller) internal {
        bool _isNormalBridging = _claim.bridgeAddrIndex < bridgingAddresses.FIRST_STAKE_BRIDGING_ADDR_IDX();
        ...
        if (_isNormalBridging && _chainWrappedTokenQuantityDestination < _wrappedTokenAmountDestination) {
        ...
        if (_quorumReached) {
            ...
            for (uint64 i = _firstTxNonce; i <= _lastTxNonce; i++) {
                ...
                if (_txType == TransactionTypesLib.NORMAL) {
                    ...
                    if (_ctx.bridgeAddrIndex < bridgingAddresses.FIRST_STAKE_BRIDGING_ADDR_IDX()) {
