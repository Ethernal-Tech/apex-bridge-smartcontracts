...
task("size-all", "Print compiled sizes of all contracts").setAction(async (_, hre) => {
  ...
  for (const fqName of artifactFiles) {
    ...
    const color = size > LIMIT ? chalk.redBright : chalk.greenBright;

    console.log(`${contractName.padEnd(30)} ${color(`${size} bytes (${sizeKB} KB)`)}`);
  }
