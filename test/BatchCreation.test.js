...
describe("Batch Creation", function () {
  describe("Batch creation", function () {
    ...
    it("Should not update nextTimeoutBlock when it is a consolidation batch", async function () {
      ...
      const tx1 = await bridge.connect(validators[0]).submitClaims(validatorClaimsBRC);
      const tx2 = await bridge.connect(validators[1]).submitClaims(validatorClaimsBRC);
      const tx3 = await bridge.connect(validators[2]).submitClaims(validatorClaimsBRC);
      const tx4 = await bridge.connect(validators[3]).submitClaims(validatorClaimsBRC);
