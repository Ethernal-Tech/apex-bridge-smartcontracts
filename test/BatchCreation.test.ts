...
describe("Batch creation", function () {
  it("SignedBatch submition should return imediatelly if chain is not registered", async function () {
    ...
    expect(await bridge.connect(validators[0]).submitSignedBatch(signedBatch_UnregisteredChain)).not.to.reverted; // submitSignedBatch should return for unregistered chain
  });
