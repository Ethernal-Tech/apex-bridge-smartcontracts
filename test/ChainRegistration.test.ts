...
describe("Chain Registration", function () {
  ...
  describe("Registering new chain with Owner", function () {
    it("Should revert new chain if not set by owner", async function () {
      ...
      await expect(
        bridge.connect(validators[0]).registerChain(chain1, validatorsCardanoData)
    ...
    it("Should emit new chain registered when registered by owner", async function () {
      ...
      await expect(bridge.connect(owner).registerChain(chain1, validatorsCardanoData))
  ...
  describe("Registering new chain with Governance", function () {
    it("Should revert proposal if chain is already registered with Governance", async function () {
      ...
      await bridge
        ...
        .registerChainGovernance(chain1.id, chain1.chainType, validatorsCardanoData[1].data);
      await bridge
        .connect(validators[2])
        .registerChainGovernance(chain1.id, chain1.chainType, validatorsCardanoData[2].data);

      expect(await claims.isChainRegistered(chain1.id)).to.be.false;

      await bridge
        .connect(validators[3])
        .registerChainGovernance(chain1.id, chain1.chainType, validatorsCardanoData[3].data);

      expect(await claims.isChainRegistered(chain1.id)).to.be.false;

      await bridge
        .connect(validators[4])
        .registerChainGovernance(chain1.id, chain1.chainType, validatorsCardanoData[4].data);
    ...
    it("Should revert proposal if not sent by validator", async function () {
      ...
      await expect(
        bridge.connect(owner).registerChainGovernance(chain1.id, chain1.chainType, validatorsCardanoData[0].data)
    ...
    it("Should list all registered chains", async function () {
      ...
      await bridge
        .connect(validators[0])
        .registerChainGovernance(chain2.id, chain2.chainType, validatorsCardanoData[0].data);
      await bridge
        .connect(validators[1])
        .registerChainGovernance(chain2.id, chain2.chainType, validatorsCardanoData[1].data);
      await bridge
        .connect(validators[2])
        .registerChainGovernance(chain2.id, chain2.chainType, validatorsCardanoData[2].data);
      await bridge
        .connect(validators[3])
        .registerChainGovernance(chain2.id, chain2.chainType, validatorsCardanoData[3].data);
      await bridge
        .connect(validators[4])
        .registerChainGovernance(chain2.id, chain2.chainType, validatorsCardanoData[4].data);
