...
describe("Claims Contract", function () {
  ...
  describe("Submit token amount data", function () {
    ...
    it("Token amount should be updated if there is consensus and slot number is higher than previous", async function () {
      const { bridge, owner, validators, chain1, validatorsCardanoData, cardanoBlocks, tokenAmounts } =
        await loadFixture(deployBridgeFixture);
      ...
      expect(await bridge.getTokenQuantity(1)).to.equal(1000000);
    });
  });
});
