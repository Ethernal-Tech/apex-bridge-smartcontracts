...
describe("Deployment", function () {
  ...
  it("setDependency should fail if any required argument is not smart contract address", async function () {
    ...
    await expect(
      bridgingAddresses
        .connect(owner)
        .setDependencies(connection.ethers.ZeroAddress, claims.getAddress(), admin.getAddress())
    ).to.be.revertedWithCustomError(validatorsc, "NotContractAddress");

    await expect(
      bridgingAddresses
        .connect(owner)
        .setDependencies(bridge.getAddress(), connection.ethers.ZeroAddress, admin.getAddress())
    ).to.be.revertedWithCustomError(validatorsc, "NotContractAddress");

    await expect(
      bridgingAddresses
        .connect(owner)
        .setDependencies(bridge.getAddress(), claims.getAddress(), connection.ethers.ZeroAddress)
    ).to.be.revertedWithCustomError(validatorsc, "NotContractAddress");
