...
import { TransactionSubType, TransactionType, deployBridgeFixture } from "./fixtures";
  ...
  describe("Stake Registration", function () {
    ...
    it("Should revert if pool id is invalid", async () => {
      await expect(
        admin
          .connect(owner)
          .stakeAddressOperation(chain1.id, bridgeAddrIndex, "none", TransactionSubType.STAKE_REGISTRATION)
    ...
    it("Should revert if index of bridging address is invalid", async () => {
      ...
      await expect(
        admin
          .connect(owner)
          .stakeAddressOperation(chain1.id, invalidBridgeAddr, stakePoolId, TransactionSubType.STAKE_REGISTRATION)
  ...
  describe("Stake Deregistration", function () {
    ...
    it("Should revert if deregistration is not sent by owner", async function () {
      await expect(
        admin
          .connect(validators[0])
          .stakeAddressOperation(
            chain1.id,
            bridgeAddrIndex,
            invalidStakePoolId,
            TransactionSubType.STAKE_DEREGISTRATION
          )
      ).to.be.revertedWith("Ownable: caller is not the owner");
    });
    ...
    it("Should increase last confirmed tx nonce when delegation is added", async function () {
      ...
      await admin
        .connect(owner)
        .stakeAddressOperation(chain1.id, bridgeAddrIndex, correctStakePoolId, TransactionSubType.STAKE_REGISTRATION);
