...
describe("Stake Registration", function () {
    ...
    it("Should revert if registration is not sent by owner", async function () {
        ...
        await expect(bridge.connect(validators[0]).stakeAddressOperation(chain1.id, bridgeAddrIndex, stakePoolId, TransactionSubType.STAKE_REGISTRATION))
    ...
    it("Should revert if chain is not registered", async () => {
        ...
        await expect(bridge.connect(owner).stakeAddressOperation(chain1.id, bridgeAddrIndex, stakePoolId, TransactionSubType.STAKE_REGISTRATION))
    ...
    it("Should revert if pool id is invalid", async () => {
        ...
        await expect(bridge.connect(owner).stakeAddressOperation(chain1.id, bridgeAddrIndex, "none", TransactionSubType.STAKE_REGISTRATION))
    ...
    it("Should revert if index of bridging address is invalid", async () => {
        ...
        await bridge.connect(owner).registerChain(chain1, 10000, 10000, validatorAddressChainData);
        await expect(bridge.connect(owner).stakeAddressOperation(chain1.id, invalidBridgeAddr, stakePoolId, TransactionSubType.STAKE_REGISTRATION))
    ...
    it("Should revert if the bridging address has already been registered.", async () => {
        ...
        await expect(bridge.connect(owner).stakeAddressOperation(chain1.id, bridgeAddrIndex, stakePoolId, TransactionSubType.STAKE_REGISTRATION))
            .to.be.revertedWithCustomError(bridge, "AddrAlreadyRegistered");
    });
    ...
    it("Should set isAddrDelegatedToStake to true when registration is successful", async function () {
        const { bridge, claims, owner, chain1, validatorAddressChainData, bridgeAddrIndex, bridgingAddresses } =
            await loadFixture(deployBridgeFixture);
...
describe("Stake Redelegation", function () {
    ...
    it("Should revert if redelegation is not sent by owner", async function () {
        ...
        await expect(bridge.connect(validators[0]).stakeAddressOperation(chain1.id, bridgeAddrIndex, stakePoolId, TransactionSubType.STAKE_DELEGATION))
            .to.be.revertedWith("Ownable: caller is not the owner");
    });
    ...
    it("Should revert if chain is not registered", async () => {
        ...
        await expect(bridge.connect(owner).stakeAddressOperation(chain1.id, bridgeAddrIndex, stakePoolId, TransactionSubType.STAKE_DELEGATION))
            .to.be.revertedWithCustomError(bridge, "ChainIsNotRegistered");
    });
    ...
    it("Should revert if the bridging address isn't registered.", async () => {
        ...
        await expect(bridge.connect(owner).stakeAddressOperation(chain1.id, bridgeAddrIndex, stakePoolId, TransactionSubType.STAKE_DELEGATION))
            .to.be.revertedWithCustomError(bridge, "AddrNotRegistered");
    });
    ...
    it("Should increase last confirmed tx nonce when delegation is added", async function () {
        ...
        await bridge.connect(owner).stakeAddressOperation(chain1.id, bridgeAddrIndex, stakePoolId, TransactionSubType.STAKE_DELEGATION);
...
describe("Stake Deregistration", function () {
    ...
    it("Should revert if deregistration is not sent by owner", async function () {
        ...
        await expect(bridge.connect(validators[0]).stakeAddressOperation(chain1.id, bridgeAddrIndex, invalidStakePoolId, TransactionSubType.STAKE_DEREGISTRATION))
            .to.be.revertedWith("Ownable: caller is not the owner");
    });
    ...
    it("Should revert if chain is not registered", async () => {
        ...
        await expect(bridge.connect(owner).stakeAddressOperation(chain1.id, bridgeAddrIndex, invalidStakePoolId, TransactionSubType.STAKE_DEREGISTRATION))
            .to.be.revertedWithCustomError(bridge, "ChainIsNotRegistered");
    });
    ...
    it("Should revert if index of bridging address is invalid", async () => {
        ...
        await bridge.connect(owner).registerChain(chain1, 10000, 10000, validatorAddressChainData);
        await expect(bridge.connect(owner).stakeAddressOperation(chain1.id, invalidBridgeAddr, invalidStakePoolId, TransactionSubType.STAKE_DEREGISTRATION))
            .to.be.revertedWithCustomError(bridge, "InvalidBridgeAddrIndex");
    });
    ...
    it("Should increase last confirmed tx nonce when delegation is added", async function () {
        ...
        await bridge.connect(owner).stakeAddressOperation(chain1.id, bridgeAddrIndex, correctStakePoolId, TransactionSubType.STAKE_REGISTRATION);

        expect(await claims.lastConfirmedTxNonce(chain1.id)).to.equal(1);

        await bridge.connect(owner).stakeAddressOperation(chain1.id, bridgeAddrIndex, invalidStakePoolId, TransactionSubType.STAKE_DEREGISTRATION);
