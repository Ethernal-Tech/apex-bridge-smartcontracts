...
describe("submitNewValidatorSet+validatorSetUpdated", function () {
    let validatorsSC: any;
    let validatorsChainDataPrime: any;
    let validatorsChainDataVector: any;
    let chains: any;
    let validatorsAddresses: string[];
    let validatorsAdditional: any;
    let contractSigner: any;

    const generateKey = function (indx: number, prefix: number): string[] {
        return [
    ...
    beforeEach(async () => {
        ...
        validatorsSC = Validators.attach(validatorsProxy.target) as Validators;
        ...
        contractSigner = await ethers.getSigner(fakeBridgeAddr);

        //To make any call with impersonated, you will need to send some eth
        const hre = require("hardhat");
        await hre.network.provider.request({
            method: "hardhat_impersonateAccount",
            params: [contractSigner.address],
        });
        await ethers.provider.send("hardhat_setBalance", [contractSigner.address, "0x56BC75E2D63100000"]);
